...
export async function checkAgentUser(
	...
	if (!agentUser) {
		return {
			check: CHECK,
			status: 'ok',
			detail: 'not applicable — no agentUser in .repo-ai.json',
		}
	}
	if (!LOGIN.test(agentUser)) {
		return {
			check: CHECK,
			status: 'drift',
			detail: `agentUser "${agentUser}" is not a valid GitHub login`,
			hint: 'Fix or remove agentUser in .repo-ai.json',
		}
	}
	...
	if (r.ok) {
		return {
			check: CHECK,
			status: 'ok',
			detail: `agentUser "${agentUser}" is an assignable collaborator`,
		}
	}
	if (/HTTP 404/.test(r.stderr)) {
		return {
			check: CHECK,
			status: 'drift',
			detail: `agentUser "${agentUser}" is not an assignable collaborator — the loop skills will silently assign nothing`,
			hint: `Add the account as a collaborator (\`gh api -X PUT repos/{owner}/{repo}/collaborators/${agentUser}\`) or remove agentUser from .repo-ai.json`,
		}
	}
